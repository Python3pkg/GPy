...
def plot_optimizer(optimizer, **kwargs):
    if optimizer.trace == None:
        ...
        plots = dict(trace=pl().plot(list(range(len(optimizer.trace))), optimizer.trace))
...
def plot_sgd_traces(optimizer):
    ...
    for k in list(optimizer.param_traces.keys()):
        plots['lines'].append(pl().plot(canvas, list(range(len(optimizer.param_traces[k]))), optimizer.param_traces[k], label=k))
    pl().add_to_canvas(canvas, legend=True)
    canvas, _ = pl().new_canvas(figure, 1, 2, title="Objective function")
    pl().plot(canvas, list(range(len(optimizer.fopt_trace))), optimizer.fopt_trace)
